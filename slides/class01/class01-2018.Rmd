...
```{r load_data}
fakestroke <- read.csv("fakestroke.csv") %>% tbl_df
bloodbrain <- read.csv("bloodbrain.csv") %>% tbl_df
```
...
Now, we'll move on to the issue of making a decision about what type of summary to show for the quantitative variables. Since the `fakestroke` data are just simulated and only match the summary statistics of the original results, not the details, we'll adopt the decisions made in the original article, which were to use medians and interquartile ranges to summarize the distributions of all of the continuous variables **except** systolic blood pressure. 
...
write.csv(fs.t1, file = "fs-table1.csv")
```

You can then open the `fs-table1.csv` file in Excel. 

## Other Approaches are described in the Course Notes
...
Consider the data, available on the Data and Code page of [our course website](https://github.com/THOMASELOVE/432-2018) in the `bloodbrain.csv` file, which includes the following variables:
...
I'll choose to treat `tlratio` as non-Normal, but otherwise, use t tests.
